...
class BackgroundListener : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        if (intent.action != null) {
            when (intent.action) {
                ProximiioAPI.ACTION_POSITION -> Log.d(
                    TAG,
                    "Position: " + intent.getDoubleExtra(ProximiioAPI.EXTRA_LAT, 0.0) + ", " + intent.getDoubleExtra(
                        ProximiioAPI.EXTRA_LON,
                        0.0
                    )
                )
                    ...
                    if (json != null) {
                        ...
                        if (title != null) {
                            ...
                            if (notificationManager != null) {
                                ...
                                val contentIntent =
                                    PendingIntent.getActivity(context, 0, intent2, PendingIntent.FLAG_CANCEL_CURRENT)
